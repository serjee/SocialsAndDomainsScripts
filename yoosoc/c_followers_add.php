<?php
/**
 * Скрипт переводит подписчиков аккаунтов в друзья.
...
foreach ($arAccounts as $user_id=>$access)
{
    ...
    $followersUser = $vkApi->usersGetFollowers($user_id);
    if(property_exists($followersUser, "response"))
    {
        if(count($followersUser->response->items)>0)
        {
            ...
            foreach($followersUser->response->items as $follower)
            {
                if($follower>0)
                {
                    $addFollowerToFriend = $vkApi->friendsAdd($follower, "");
                    sleep(2); // задержка 2 секунды перед каждым добавлением, иначе будет ошибка
                }
            }
        }
    }
